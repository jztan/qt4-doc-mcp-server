#!/usr/bin/env python3
"""
Release automation script for qt4-doc-mcp-server using gitflow.

Usage:
    python scripts/release.py [patch|minor|major] [--dry-run]

Examples:
    python scripts/release.py patch           # 0.5.0 -> 0.5.1
    python scripts/release.py minor           # 0.5.0 -> 0.6.0
    python scripts/release.py major           # 0.5.0 -> 1.0.0
...
PYPI_PACKAGE = "qt4-doc-mcp-server"
GITHUB_REPO = "jztan/qt4-doc-mcp-server"
...
def preflight_checks() -> None:
    ...
    if branch != "develop":
        print(
            "Error: Must be on 'develop' branch to start release, "
    ...
    # Dependency audit, using the same script CI runs, so a green preflight
    # means a green publish-pypi job. Catches the failure mode where
    # pip-audit blocks the release after the tag has already been pushed.
    ...
    if result.returncode != 0:
        print(
            ...
            "Fix or update the ignore list before releasing."
        )
        print(result.stdout)
        print(result.stderr)
        sys.exit(1)
    ...
    if result.returncode != 0:
        ...
        sys.exit(1)
    # Check gh is authenticated, prompt login if not
    ...
    if result.returncode != 0:
        print(
            "Warning: 'mcp-publisher' not found. MCP Registry publish will be skipped."
        )
...
def update_init_py(project_root: Path, new_version: str, dry_run: bool) -> None:
    ...
    init_py = project_root / "src" / "qt4_doc_mcp_server" / "__init__.py"
...
def update_changelog(project_root: Path, new_version: str, dry_run: bool) -> None:
    ...
    # so fail loud instead and have the user fill it in.
...
```bash
pip install {PYPI_PACKAGE}=={new_version}
```

## Links
- [PyPI Package](https://pypi.org/project/{PYPI_PACKAGE}/{new_version}/)
- [MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search={PYPI_PACKAGE})
...
def build_raw_release_body(changelog_section: str, new_version: str) -> str:
    """Raw-changelog notes format (fallback path)."""
    tag = f"v{new_version}"
...
You are writing the GitHub release notes for qt4-doc-mcp-server {tag},
an offline MCP server that serves Qt 4.8.4 documentation to AI agents
and IDEs, with Markdown conversion and full-text search.
...
def wait_for_pypi(new_version: str, max_wait: int = 600) -> bool:
    ...
    url = f"https://pypi.org/pypi/{PYPI_PACKAGE}/{new_version}/json"
    start_time = time.time()
    check_interval = 15
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Release automation for qt4-doc-mcp-server (gitflow)",
...
Gitflow:
  develop -> release/vX.Y.Z -> master (tagged) -> merge back to develop
    ...
    print("=" * 60)
    print("  qt4-doc-mcp-server Release Automation (Gitflow)")
    print("=" * 60)
    ...
    # Step 7: Watch the publish-pypi workflow. The GitHub release is created
    # AFTER PyPI confirms; otherwise the release notes ship a `pip install`
